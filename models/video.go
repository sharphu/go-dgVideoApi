...
import (
	...
	redisClient "go-dgVideoApi/services/redis"
	"strconv"
	"time"
)
...
func SaveVideo(title string, subTitle string, channelId int, regionId int, typeId int, playUrl string, user_id int, aliyunVideoId string) error {
	o := orm.NewOrm()
	var video Video
	v_time := time.Now().Unix()
	video.Title = title
	video.SubTitle = subTitle
	video.AddTime = v_time
	video.Img = ""
	video.Img1 = ""
	...
	video.TypeId = typeId
	video.EpisodesUpdateTime = v_time
	video.Comment = 0
	...
	if err == nil {
		...
		_, err = o.Raw("INSERT INTO video_episodes (title,add_time,num,video_id,play_url,status,comment,aliyun_video_id) VALUES (?,?,?,?,?,?,?,?)", subTitle, v_time, 1, videoId, playUrl, 1, 0, aliyunVideoId).Exec()
	}
